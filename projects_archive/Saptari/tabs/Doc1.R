...
AC_land_for_cultivation_baseline <- rbind(xland_Saptari17,xland_Rautahat_Bara_Sarlahi18)
...
xlands_IHH <- AC_land_for_cultivation_baseline %>% 
...
filter(!HH %in% c("A104507035","E0104705010","A0110402001"))
  ...
  filter(!HH %in% c("A104507035","E0104705010","A0110402001")) %>% 
  group_by(district) %>% 
  summarise(`Area size (in ha)`=mean(`Net Cropped Area`)) %>% 
  mutate( data="Total land for \ncultivation (survey)",
          Seasons= "Total land") %>% mutate(`Area size (in ha)`*0.0339)
...
x <- xagri_mo %>% group_by(district,Seasons) %>%  
  summarise(m=mean(`Area Irrigated (monitoring)`)) %>% 
  filter(!Seasons %in% c("Annual 2017-2018", "Annual 2018-2019")) %>% 
  summarise(mean(m))

mean(xagri_mo$`Area Irrigated (monitoring)`)
...
Procurement <- 
    ...
    mutate(district=ifelse(district == "SAPTARI","Saptari",
                           ifelse(district %in% c("1","2","3"),"Rautahat Bara Sarlahi", district))) 
  
  
Procurement %>% 
  select(year,district,household_questionnaire_id,total_litres_consumed_dieselkero) %>%
  filter(!is.na(total_litres_consumed_dieselkero)) %>%
  group_by(district, year) %>% count() 
  
my_basket <- 
  data.frame(District = c("Rautahat Bara Sarlahi","Rautahat Bara Sarlahi",
                          "Rautahat Bara Sarlahi","Rautahat Bara Sarlahi",
                          "Saptari","Saptari","Saptari",
                          "Saptari","Saptari","Saptari"), 
             year = c("01","01","02","02","01","01","02","02","03","03"),
             phil = c("pmp1","back1","pmp1","back1","pmp","back","pmp","back","pmp","back"),
             diesel_pumps = c(27,3,16,14,
                              10,13,10,13,7,16)) 

  ggplot(my_basket,aes(x=year,y=diesel_pumps,label=phil, fill=phil) )+
  geom_bar(stat = "identity",alpha=.75) +
  theme_minimal()+
  labs(x = " ",y="Number of diesel pumps",title = "Number of diesel pumps in use")+
  facet_grid(. ~ District) + 
  geom_text(aes( y = diesel_pumps,label = diesel_pumps),
            vjust= 1.4, color= "black", size = 7)+
  theme( panel.grid.minor = element_blank(),legend.position = "none",
         text = element_text(family = "Times New Roman"),
         axis.text.y = element_text(size = (17)),
         axis.title.y = element_text(size = 20),
         strip.text.x = element_text(size = 20))+
  scale_fill_manual(values=c("lightsalmon2", "darkolivegreen3",
                             "lightsalmon4","darkolivegreen4"))+
   scale_y_continuous(breaks=c(0,30),
      sec.axis = sec_axis(~ . * 1, breaks=c(0,23)))
...
#  filter(district == "Rautahat_Bara_Sarlahi") %>% 
  select(year,district,household_questionnaire_id,total_litres_consumed_dieselkero) %>%
  spread(year, total_litres_consumed_dieselkero)
view(x)


x <- Procurement %>%
#  filter(district != "Rautahat_Bara_Sarlahi",year==2019) %>% 
  select(year,district,household_questionnaire_id,total_litres_consumed_dieselkero,do_you_buy_fuel_for_the_pump,how_many_electric_pumps_do_yo) 
view(x) 


# How many electric pumps do you operate using this connection?
how_many_electric_pumps_do_yo <- 
  Procurement %>% 
  select(year,district,household_questionnaire_id,how_many_electric_pumps_do_yo) %>% 
  drop_na(how_many_electric_pumps_do_yo) %>% 
  group_by(year,district) %>% count()
view(how_many_electric_pumps_do_yo)
...
aqua_sap <- 
  lands_I_17_18_19 %>%
  filter(TC==1) %>% 
#  filter(!household_questionnaire_id %in% c("T109205004")) %>% 
...
six_crop_sap <-
  Agriculture_17_18_19 %>% 
  filter(TC==1, !name_of_crop %in% c("Cereals","Others")) %>% 
  select(household_questionnaire_id,year,season_of_crop,name_of_crop,cult_area_under_crop) %>% 
#  filter(household_questionnaire_id %in% c("T109205004")) %>% 
  group_by(household_questionnaire_id,year,name_of_crop) %>% 
  summarise(cult_area_under_crop=sum(cult_area_under_crop*0.0339)) %>% 
  spread(name_of_crop,cult_area_under_crop) %>%
  gather(name_of_crop,cult_area_under_crop,3:8) %>%
  replace_na(list(cult_area_under_crop = 0)) %>% 
  bind_rows(aqua_sap) %>% 
  group_by(year,name_of_crop) %>% 
  summarise(mean_cult_area_under_crop=mean(cult_area_under_crop)) %>% 
  mutate(mean_cult_area_under_crop=round(mean_cult_area_under_crop, 2),
         District= "Saptari")
...
cropping_pattern <- 
  bind_rows(six_crop_sap,six_crop_rbs)
1000/800
ggplot(cropping_pattern, aes(x = year, y = mean_cult_area_under_crop, fill = name_of_crop, label = mean_cult_area_under_crop)) +
  ...
  theme(axis.text.x = element_text(size = (20)),axis.text.y = element_text(size = (17)),
...
# Cropping Pattern by season ----
#aquaculture
total_aqua_rbs <- 
  lands_I_18_19 %>% filter(TC==1) %>%
  select(year,household_questionnaire_id,land_for_aquaculture_ponds) %>%
  mutate(land_for_aquaculture_ponds=land_for_aquaculture_ponds*0.0339,
         district = "Rautahat, Bara & Sarlahi")

total_aqua <- 
  lands_I_17_18_19 %>% filter(TC==1) %>%
  select(year,household_questionnaire_id,land_for_aquaculture_ponds) %>%
  mutate(land_for_aquaculture_ponds=land_for_aquaculture_ponds*0.0339,
         district = "Saptari") %>% 
  bind_rows(total_aqua_rbs)

#Land
total_irri_rbs <- 
  Land_18_19 %>% filter(TC==1) %>% 
  mutate(total_land_cultivated=total_land_cultivated*0.0339) %>% 
  select(year,household_questionnaire_id,season,total_land_cultivated) %>%
  spread("season","total_land_cultivated") %>% 
  mutate(district = "Rautahat, Bara & Sarlahi")
  

total_irri <- 
  land_17_18_19 %>%filter(TC==1) %>% 
  mutate(total_land_cultivated=total_land_cultivated*0.0339) %>%
  select(year,household_questionnaire_id,season,total_land_cultivated) %>%
  spread("season","total_land_cultivated") %>% 
  mutate(district = "Saptari") %>% replace_na(list(Annual=0)) %>% 
  bind_rows(total_irri_rbs) %>% 
  full_join(total_aqua) %>% 
  mutate(summer = Summer+Annual+land_for_aquaculture_ponds,
         monsoon = Monsoon+Annual+land_for_aquaculture_ponds,
         winter = Winter+Annual+land_for_aquaculture_ponds) %>% 
  filter(!household_questionnaire_id %in% c("T300608033","T309708020")) %>% # HH who practice mainly fish and in a large scle
  group_by(district,year) %>% 
  summarise(summer=mean(summer),monsoon = mean(monsoon),winter= mean(winter)) %>% 
  gather("season","size" ,3:5) %>% 
  mutate(across(is.numeric, round, 2)) %>% 
  mutate(Season= case_when(year==2017 & season == "summer" ~ "summer_2017",
                           year==2018 & season == "summer" ~ "summer_2018",
                           year==2019 & season == "summer" ~ "summer_2019",
                           
                           year==2017 & season == "monsoon" ~ "monsoon_2017",
                           year==2018 & season == "monsoon" ~ "monsoon_2018",
                           year==2019 & season == "monsoon" ~ "monsoon_2019",
                           
                           year==2017 & season == "winter" ~ "winter_2017",
                           year==2018 & season == "winter" ~ "winter_2018",
                           year==2019 & season == "winter" ~ "winter_2019"
                           )) %>% 
  mutate(size=ifelse(district=="Rautahat, Bara & Sarlahi" & Season=="winter_2019",3.20,size))




  ggplot(total_irri,aes(Season,size, fill=Season))+
  geom_col(position="dodge",width= 0.75,alpha=.65) +
  geom_text(aes(label = size), position = position_dodge(0.70), vjust= 1.25, color= "black", size = 4)+
  facet_grid(. ~ district) +
  ...
  theme(panel.grid.minor = element_blank(),text = element_text(family = "Times New Roman")) +
  scale_x_discrete(limits = c("monsoon_2017","winter_2017","summer_2017",
                              "monsoon_2018","winter_2018","summer_2018",
                              "monsoon_2019","winter_2019","summer_2019"))+ 
    scale_fill_manual(values=c("dimgrey","dimgrey","dimgrey",
...
aquaculture2.0 <- 
  ...
  group_by(district,year) %>% 
  summarise(Mean=mean(total_area_of_pond)*0.0339,n())
...
aq3 <- aqua_rbs %>% filter(cult_area_under_crop != 0) %>% count(name_of_crop,year)
Ag3 <- Agriculture_18_19 %>% filter(TC==1) %>% 
  select(household_questionnaire_id ,name_of_crop, year) %>% 
  distinct() %>% 
  group_by(year) %>% 
  count(name_of_crop) %>% bind_rows(aq3) %>% 
  spread(year,n) %>% 
  rename("18" = `2018`,"19."=`2019`)

aq3 <- aqua_sap %>% filter(cult_area_under_crop != 0) %>% count(name_of_crop,year)
Ag1 <- Agriculture_17_18_19 %>% filter(TC==1) %>% 
  select(household_questionnaire_id ,name_of_crop, year) %>% 
  distinct() %>% 
  group_by(year) %>% 
  count(name_of_crop) %>% bind_rows(aq3) %>% spread(year,n)
