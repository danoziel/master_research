...
time_energy_5days_gap <-
  NYgaps_irrigation %>% mutate(btwn=lag(gap)) %>%
  filter(Irrigation!="No") %>% select(1,2,3,5,7) %>%
  rename(date=start) %>%
  left_join(water01) %>%
  select(1,2,3,5,13,26) %>%
  rename(time=Hours,flow_meter_reading=DIFFERENCE) %>%
  filter(gap>5,flow_meter_reading>0)

time_energy_10days_gap <-
  NYgaps_irrigation %>% mutate(btwn=lag(gap)) %>%
  filter(Irrigation!="No") %>% select(1,2,3,5,7) %>%
  rename(date=start) %>%
  left_join(water01) %>%
  select(1,2,3,5,13,26) %>%
  rename(time=Hours,flow_meter_reading=DIFFERENCE) %>%
  filter(gap>10,flow_meter_reading>0)
  
ggplot(time_energy_5days_gap, aes(x = time, y=flow_meter_reading, color=district, shape=district)) +
  geom_point(size = .85) + 
