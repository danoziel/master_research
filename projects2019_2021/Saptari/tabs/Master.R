...
num_days_in_season <- 
  ...
  distinct() %>% count(season) %>% 
  rename(num_days_in_season=n)

water01_SEASONs <- inner_join(water01_SEASONs,num_days_in_season)  
...
# --------------------------------------------------- -----------
  #  Irrigation Days ##                                         ----
# first year of every HH

A110402001 == A0110402001
...
wa1 <- 
  ...
  group_by(HH) %>% count() %>% mutate(fy=365)
...
ggplot(days_use_hh) + 
  ...
  geom_vline(aes(xintercept = mean(percentage)),linetype = "dashed", size = 1.25)+
  labs(title = "Percentage of SPIP use for HH",
       x="Percentages of usage days", y="Households frequency")+
  scale_x_continuous(breaks = seq(0, 1, 0.1))+
  theme_minimal() +
...
total_hours_usege <- 
  ...
  summarise(n(),`Total Hours`=mean(Hours)) %>% 
  mutate(across (is.numeric,round))
kable(total_hours_usege) %>% kable_styling()
...
normal_to_day_hours_usege <- 
  water01_SEASONs %>% 
  group_by(district, season,HH) %>%  
  summarise(Hours=sum(Hours)) %>% 
  group_by(district,season) %>%  
  summarise(n(),`Total Hours`=mean(Hours)) %>% 
  inner_join(num_days_in_season) %>% 
  mutate(`Total Hours`/num_days_in_season)
  mutate(across (is.numeric,round))

kable(normal_to_day_hours_usege) %>% kable_styling()
...
# crop_freq ----
crop_freq_s <- 
  Agriculture_17_18_19 %>% filter(year==2017,TC==1) %>% select(household_questionnaire_id,name_of_crop) 

crop_freq_rbs <- 
  Agriculture_18_19 %>% filter(year==2018,TC==1) %>% select(household_questionnaire_id,name_of_crop) 

crop_freq_aqua <- 
  aquaculture %>% filter(TC==1,year==2018) %>% mutate(name_of_crop="Fish farming") %>% select(household_questionnaire_id,name_of_crop) 

crop_freq <- rbind(crop_freq_s,crop_freq_rbs) %>% 
  full_join(crop_freq_aqua) %>%
  mutate(paddy=ifelse(name_of_crop=="Paddy",1,0),
         wheat=ifelse(name_of_crop=="Wheat",1,0),
         vegetables=ifelse(name_of_crop=="Vegetables",1,0),
         pulses=ifelse(name_of_crop=="Pulses",1,0),
         sugarcane=ifelse(name_of_crop=="Sugarcane",1,0),
         fish_farming=ifelse(name_of_crop=="Fish farming",1,0),
         maize=ifelse(name_of_crop=="Maize",1,0),
         oilseeds=ifelse(name_of_crop=="Oilseeds",1,0)) %>%select(-2) %>%  
  group_by(household_questionnaire_id) %>% distinct()%>% 
  summarise(paddy=sum(paddy),wheat=sum(wheat),vegetables=sum(vegetables),
            pulses=sum(pulses),oilseeds=sum(oilseeds),fish_farming=sum(fish_farming),
            maize=sum(maize),sugarcane=sum(sugarcane)) %>% 
  rename(HH=household_questionnaire_id) %>% 
  inner_join(days_use_hh) %>% select(-c(total_days,irrigation_days))

sr$irradiance <- as.factor(sr$irradiance)
sr$irradiance<- relevel(sr$irradiance, ref = "low")

mod01 <- lm(percentage ~ pulses,crop_freq)

tab_model(mod01,digits=3,p.style="numeric",show.se = TRUE,
          string.ci = "Conf. Int (95%)",dv.labels = c(" "))
#          pred.labels = c("(Intercept)", "Solar Radiation (GHI)"))

 #paddy+wheat+vegetables+pulses+oilseeds+fish_farming+maize+sugarcane
